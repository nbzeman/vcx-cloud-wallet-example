...
echo "Enter the absolute path to the genesis.txn file (the default assumes you are using Vagrant, and will use /home/vcx-vagrant/config/genesis.txn"
read -p ':' genesispath
echo $genesispath
...
if [ -z "$genesispath" ]
then
      echo "\$genesispath is empty, using default value of /home/vagrant/vcx-vagrant/config/genesis.txn"
      genesispath="../config/genesis.txn"
...
sudo dpkg -i libindy_1.15*.deb &&
sudo dpkg -i libsovtoken_1.0.5*.deb &&
sudo dpkg -i libvcx_0.8*.deb &&
sudo apt-get install -f &&

#  Provision Keys and Build Wallet, copy values to config/vcx-config.json (if wallet already exists remove it first)
if [  -d "/home/vagrant/vcx-vagrant/.indy_client/wallet" ]; then
...
tar xvf python3-vcx-wrapper_0.8.72143223.tar.gz &&
if [ ! -d "../server/modules" ]; then
    mv python3-vcx-wrapper-0.8.72143223 ../server/modules &&
    rm -r python3-vcx-wrapper-0.8.72143223 &&
...
sudo chmod +x VCXWebApp-win.service &&
sudo cp VCXWebApp-win.service /etc/systemd/system/VCXWebApp-win.service &&
sudo systemctl start VCXWebApp-win.service &&
# install and config nginx
yes | sudo apt-get install nginx &&
# clean /var/www
sudo rm -Rf /var/www/html/* &&
# symlink /var/www => /home/vagrant/vcx-vagrant/web
sudo ln -s /home/vagrant/vcx-vagrant/web/* /var/www/html &&
sudo cp ../web/default /etc/nginx/sites-available/default
# restart services
sudo systemctl restart nginx.service
sudo systemctl restart VCXWebApp-win.service &&
sudo systemctl status VCXWebApp-win.service &&
# test VCX
python3 editConfig.py "$imageurl" "$instname" "$genesispath" &&
