...
// This is currently an unused file, 
// that is being developed as a merge between the currently
// encapsulated vcx-server.js and vcx-web-tools.js system. 
...
async function getConnection(name){
  let connection_serialized = await fs.readJSON(`../data/${name}-connection.json`);
  ...
  for(let f of file_list){
    if(f.name.includes('connection')){
      ...
      if(cFile['data']['their_pw_did']==did){
        console.log(`redirected did:  ${did} is equal to existing did: ${cFile['data']['their_pw_did']}`);
...
async function askProvableQuestion (connection, qtext) {
    if(connection == null){
      console.log("Connection Was Not Found in Records. Please Delete Your Connection and Try Again.");
    }else{
...
async function offerCredential(credential_name,connection){
    ...
    while(state != StateType.Accepted && timer < 200) {
      console.log("Credential Sent, The State of the Credential is "+ state +"timer "+ timer);
      await sleep(2000);
      await credential.updateState();
      state = await credential.getState();
      timer+=1;
...
let receiveConnection = async function(id, invite_details){
  let requestedConnection = await Connection.createWithInvite({"id":id,"invite":invite_details});
...
let receiveCredential = async function(connection){
  let offers = await Credential.getOffers(connection);
  let credential = await Credential.create({ "sourceId": "received_credential", "offer": JSON.stringify(offers[0]), "connection": connection});
  await credential.sendRequest({ "connection": connection, payment: "0"});
  let credentialState = await credential.getState();
  while (credentialState !== StateType.Accepted) {
    await credential.updateState();
