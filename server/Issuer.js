...
const webhookUrl = "http://bb4c63587890.ngrok.io/webhook"; // public URL for the webhook endpoint
...
async function registerDid(issuerDid, issuerVerkey) {
  console.log(`Posting message to register DID`);
...
async function issuer() {
  ...
  const updateConfigs = new Promise(function (resolve, reject) {
  ...
  const getIssuerKeys = new Promise(function (resolve, reject) {
  ...
  if (fs.existsSync(CRED_DEF_FILE)) {
    ...
    console.log(
      `Created credential definition: ${ANSII_GREEN}${credDefId}${ANSII_RESET}`
    );
  ...
  const response = callAcceptInviteApi(inviteUrl);
...
async function callAcceptInviteApi(inviteUrl) {
  const data = {
    protocol: "outOfBand",
    connection_invite: inviteUrl,
    name: "test",
  };
  console.log("Sending invite url");
  console.log(data);
  return axios({
    method: "POST",
    url: `${walletServerUrl}/api/v1/accept_invite`,
    data: data,
  });
}
